...
static enum action select_up_down(void)
{
	...
	if ((c0_freq < (down_freq + FREQ_OFFSET)) && (c1_freq < down_freq)) {
...
static enum hstate hotplug_adjust_state(enum action move)
{
	...
	if (move == DOWN) {
		if (ctrl_hotplug.old_state == H0 &&
		    ctrl_hotplug.tasks < nr)
...
define_show_state_function(tasks)
define_store_state_function(tasks)
...
static DEVICE_ATTR(tasks, S_IRUGO | S_IWUSR, show_tasks, store_tasks);
