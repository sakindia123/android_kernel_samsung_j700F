...
static int exynos_tmu_initialize(struct platform_device *pdev)
{
	...
	if ((data->soc == SOC_ARCH_EXYNOS7580) && data->id) {
		if (pdata->d_type == GPU && (!(trim_info >> 24) & 0x1)) {
			trim_info = readl(data->base_second + reg->triminfo_data);
		} else if (pdata->d_type == CPU) {
			if (!((readl(data->base_second + reg->triminfo_data) >> 24) & 0x1))
				trim_info = readl(data->base_second + reg->triminfo_data);
		}
	}
		...
		if (pdata->trigger_levels[i] &&
			...
			if (i == EXYNOS_MAX_TRIGGER_PER_REG - 1) {
				if (config_enabled(CONFIG_SOC_EXYNOS7580)) {
					/* 7th level to be assigned in th2 reg */
					rising7_4_threshold &= ~(0xff << reg->threshold_th3_l0_shift);
					rising7_4_threshold |=
...
static int exynos_tmu_read(struct exynos_tmu_data *data)
{
	...
	if (IS_ENABLED(CONFIG_SOC_EXYNOS7580)) {
		if (!(temp_code >> EXYNOS7580_TMU_VALID_P0) & 0x1) {
			pr_debug("Invalid temperature value\n");
			temp = -EINVAL;
			goto out;
		}
	}
			...
			struct platform_device *pdev, int id)
{
	...
	if (soc_is_exynos7580_v1())
		data_table = (struct exynos_tmu_init_data *) EXYNOS7580_TMU_DRV_DATA_V1;
	else
#endif
